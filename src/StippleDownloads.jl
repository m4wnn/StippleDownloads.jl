...
function _download_binary(model::ReactiveModel, data, filename = "file.bin", array_type::Type{<:Real} = UInt8; client::Union{Nothing,UInt,Vector{UInt}} = nothing)
...
function download_binary(model::ReactiveModel, data, filename = "file.bin", array_type::Type{<:Real} = UInt8; client::Union{Nothing,UInt,Vector{UInt}} = nothing)
    _download_binary(model, data, filename, array_type; client)
end

function download_binary(model::ReactiveModel, data::Vector{T}, filename = "file.bin", array_type::Type{<:Real} = T; client::Union{Nothing,UInt,Vector{UInt}} = nothing) where T <: Real
    _download_binary(model, data, filename, array_type; client)
end
